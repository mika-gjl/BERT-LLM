...
class RelCombDataset(torch.utils.data.Dataset):
    ...
    def gettext(self, x): return x['sent']
    def getspos(self, x): return x['triples']
    def gets(self, x): return TN(x['sub'])
    def geto(self, x): return TN(x['obj'])
    def getr(self, x): return x['rel']
    def tokenize(self, x): 
