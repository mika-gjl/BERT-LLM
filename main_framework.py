...
parser.add_argument("--dname", help='dataset for training and testing', choices=['pegazus-event-extraction'], default='pegazus-event-extraction')
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data, negative_threshold):
        ...
        for z in data:
            item = {}
            item['tid'] = torch.tensor(tokenizer.encode(z['sent'])[:512])
            item['yrc'] = list(set(rel_map[x['rel']] for x in z['triples']))
...
class DatasetEE(torch.utils.data.Dataset):
    def __init__(self, data, negative_threshold):
        ...
        for i, z in enumerate(data):
            text, spo_list = z['sent'], z['triples']
            labels = z.get('rc_pred', list(set(x['rel'] for x in spo_list)))  
            ...
            for label in labels:
                ...
                ss = set(TN(x['sub']) for x in spo_list if x['rel'] == label)
                oo = set(TN(x['obj']) for x in spo_list if x['rel'] == label)
...
class DatasetonlyEE(torch.utils.data.Dataset):
    def __init__(self, data, negative_threshold):
        ...
        for i, z in enumerate(data):
            ...
            slen = item['tid'].size(0)
            ss = set(TN(x['sub']) for x in spo_list)
            oo = set(TN(x['obj']) for x in spo_list)
            yy = torch.zeros((slen, 4)).float()
            for s in ss:
                for u, v in FindValuePos(otokens, s): 
                    yy[u,0] = yy[v-1,1] = 1
...
def ComputeOne(item, pairs, f1, fout, label=None):
    # spos = [x for x in item['spo_list'] if x['rel'] == item['rel']]
    spos = [{'sub': x['sub'], 'obj':x['obj']} for x in item['spo_list']]
...
def decode_entitypair(item, rr, ee_threshold, gpout=None):
    ...
    for sv1, sv2 in [(sv1, sv2) for sv1 in vv1 for sv2 in vv2]:
        ...
        pairs.append({'sub': sv1, 'obj':sv2})
    return pairs

def tt2(t):
    try:
        ans = t['sub']+' | '+t['rel']+' | '+t['obj']
    except:
        ans = 'wrongcase'
    return ans
...
if args.cons_candidate:
    ...
    with open(wdir('candidate.json'), 'w', encoding='utf-8') as fout:
        wdata = [{'id': x['id'], 'sent':x['sent'],'preds':x['preds'],'std_ans':x['triples']} for x in tdata] 
